 ...
 * authentication on custom domains and hosted deployments without requiring
...
export function clerkProxyMiddleware(): RequestHandler {
  ...
  return createProxyMiddleware({
    ...
    on: {
      ...
      // Transfer-Encoding: chunked — which the deployment edge rejects, turning
      // the app's 200 into a 500. Buffer only those so they can be re-sent with
      // a Content-Length; the body is forwarded untouched so Content-Encoding is
      // preserved. Length-known responses and body-less responses stream through
      // without buffering.
